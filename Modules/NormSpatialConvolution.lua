local NormSpatialConvolution, parent =
   torch.class('cudnn.NormSpatialConvolution', 'nn.SpatialConvolution')
local ffi = require 'ffi'
local errcheck = cudnn.errcheck

function NormSpatialConvolution:__init(nInputPlane, nOutputPlane,
...
function NormSpatialConvolution:resetWeightDescriptors()
...
function NormSpatialConvolution:createIODescriptors(input)
   local batch = true
   if input:dim() == 3 then
...
function NormSpatialConvolution:norm_filters() 
    local w = self.weight 
    local wsz = w:size()
...
function NormSpatialConvolution:updateOutput(input)
...
function NormSpatialConvolution:updateGradInput(input, gradOutput)
...
function NormSpatialConvolution:accGradParameters(input, gradOutput, scale)
...
--[[
function NormSpatialConvolution:zeroGradParameters()
